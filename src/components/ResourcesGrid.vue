        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-wrap gap-3 mb-12">
                <button @click="setFilter('all')"
                    class="px-6 py-2 rounded-full transition-all duration-300 text-sm md:text-base"
...
// Utility function (Must be defined for the template to work)
const getRoutePath = (type, slug) => {
    // Logic to build the URL: e.g., /project/my-slug or /blog/my-slug
    // We remove '-study' from 'case-study' to match a typical route of /case/slug
    return `/${type.replace('-study', '')}/${slug}`
}
    ...
    try {
        ...
        allResources.value = [
            ...(caseStudies?.data ?? []).map(item => ({
                id: item.id,
                type: 'case-study',
                title: item.title, // ✅ Correct: item.title
                label: item.category || 'Case Study', // ✅ Correct: item.category
                slug: item.slug,   // ✅ Correct: item.slug
                thumbnail: item.heroImage?.data?.attributes?.url
                    ? `${STRAPI_URL}${item.heroImage.data.url}`
                    : '',
            })),
            ...(projects?.data ?? []).map(item => ({
                ...
                type: 'project',
                title: item.title, // ✅ Correct: item.title
                label: item.category || 'Project', // ✅ Correct: item.category
                slug: item.slug,   // ✅ Correct: item.slug
                thumbnail: item.heroImage?.data?.attributes?.url
                    ? `${STRAPI_URL}${item.heroImage.data.url}`
                    : '',
            ...
            ...(blogs?.data ?? []).map(item => ({
                ...
                type: 'blog',
                title: item.title, // ✅ Correct: item.title
                label: item.category || 'Blog', // ✅ Correct: item.category
                slug: item.slug,   // ✅ Correct: item.slug
                thumbnail: item.heroImage?.data?.attributes?.url
                    ? `${STRAPI_URL}${item.heroImage.data.url}`
                    : '',
...
const filteredResources = computed(() => {
    if (!allResources.value || allResources.value.length === 0) return []
    if (activeFilter.value === 'all') return allResources.value
    return allResources.value.filter(r => r.type === activeFilter.value)
})
